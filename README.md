...
本项目（Alpine-Sing-box）仅供学习、研究和技术交流使用，旨在帮助用户在 Alpine Linux 上部署 sing-box 透明网关并理解其工作原理。

**使用本项目前请知悉以下条款：**

1. **合规使用**：用户应确保在使用本项目时遵守所在国家/地区的法律法规。因使用本项目产生的任何法律责任由用户自行承担，与本项目开发者及贡献者无关。
2. **仅供学习**：本项目不提供、不分发任何代理节点或订阅链接，也不鼓励或协助用户绕过任何网络审查或访问受限内容。用户需自行准备合法的代理服务。
3. **不作担保**：本项目按「现状」（AS IS）提供，不提供任何明示或暗示的担保，包括但不限于适销性、特定用途适用性和不侵权的担保。
4. **风险自担**：本项目涉及系统网络配置（nftables、路由表、内核模块、DNS），操作不当可能导致网络中断或系统异常。用户应在充分理解脚本内容后自行决定是否使用，所有风险由用户自行承担。
5. **第三方内容**：本项目引用了 sing-box（[SagerNet/sing-box](https://github.com/SagerNet/sing-box)）及其规则集（sing-geosite / sing-geoip）等第三方开源项目，相关权利归原作者所有。本项目不对第三方项目的可用性、安全性或合规性负责。
6. **无关联声明**：本项目与 sing-box 官方、Clash 官方、MetaCubeX 等组织无任何隶属或关联关系。

**下载或使用本项目即表示您已阅读并同意以上免责声明。如果您不同意，请立即停止使用并删除本项目。**
